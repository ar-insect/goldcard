...
TabPanel.prototype = {
    ...
    init: function () {
        ...
        this.tabpanel_left_scroll.addClass('tabpanel_left_scroll glyphicon glyphicon-arrow-left');
        ...
        this.tabpanel_right_scroll.addClass('tabpanel_right_scroll glyphicon glyphicon-arrow-right');
...
*/
    ...
    iterateFlush: function (iframeObj) {
        var ifrwin = iframeObj.contentWindow;
        var ifrdoc = iframeObj.contentDocument || ifrwin.document;
        if (ifrwin.frames.length > 0) {
            for (var i = 0; i < ifrwin.frames.length; i++) {
                this.iterateFlush(ifrwin.frames[i]);
            }
        }
        else {
            if (ifrdoc.forms.length > 0) {
                for (var i = 0; i < ifrdoc.forms.length; i++) {
                    try {
                        ifrdoc.forms[i].submit();
                    }
                    catch (e) {
    ...
    kill: function (position) {
        ...
        //remove from tabs
        var undock = this.tabs.splice(position, 1);
        // add kill callback
        if ($.isFunction(this.killCallback)) {
            this.killCallback.call(this, undock);
        }
